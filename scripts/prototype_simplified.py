...
def infer(expr: Node, ctx: dict[str, Forall]) -> MonoType:
    ...
    if isinstance(expr, Where):
        ...
        body_ty = infer(body, {**ctx, name: Forall([], value_ty)})
...
if __name__ == "__main__":
    ...
    expr = Where(Assign(Var("y"), Int()),
                Where(Assign(Var("f"), Function(Var("y"), Var("y"))),
                    Where(Assign(Var("x"), Apply(Var("f"), Bool())),
                        Var("x")))
    )
